...
async function fetchOffersPage(page) {
  const targetUrl = page > 1 ? `https://www.mercadolivre.com.br/ofertas?page=${page}` : 'https://www.mercadolivre.com.br/ofertas';
  
  const apiKey = process.env.SCRAPER_API_KEY;
  if (!apiKey) throw new Error('SCRAPER_API_KEY não configurada. O ScraperAPI é necessário para buscar ofertas do Mercado Livre.');
...
export async function searchOffers(workspaceId, filters = {}) {
  ...
  try {
    // Buscar 2 páginas de ofertas (aprox 96 ofertas)
