...
export async function searchOffers(workspaceId, filters = {}) {
  ...
  const keywords = keyword ? keyword.split(',').map((k) => k.trim()).filter(Boolean) : ['ofertas'];
  const maxKeywords = keywords.slice(0, 5); // Limita a 5 buscas simultâneas

  const fetchPromises = maxKeywords.map(async (kw) => {
    try {
      const url = `${KABUM_API}/products?query=${encodeURIComponent(kw)}&page_number=1&page_size=${Math.min(limit, 30)}`;
  ...
  // Remove duplicatas
  const uniqueResults = Array.from(new Map(allResults.map(item => [item.code || item.id, item])).values());
